...
void PhysicsList::ConstructProcess()
{
  ...
  //param->SetActivation(false);  // 禁用所有电磁过程
  // param->SetMinEnergy(1.*keV);
  // param->SetMaxEnergy(120.*keV);
  param->SetNumberOfBinsPerDecade(10);
  param->SetVerbose(1);
  //param->SetLowestElectronEnergy(0.*keV);

  // Enable or disable specific processes using G4ProcessManager
  ...
  //   pManager->SetProcessActivation(i, false);  // 禁用所有物理过程
  // } 

  // Iterate over the processes to find the desired ones
  // G4int comptonIndex = -1;
  // G4int photoIndex = -1;
  // G4int pairIndex = -1;
  ...
  //     comptonIndex = static_cast<G4int>(i);
  //   } else if (process->GetProcessName() == "phot") {
  //     photoIndex = static_cast<G4int>(i);
  //   } else if (process->GetProcessName() == "ePair") {
  //     pairIndex = static_cast<G4int>(i);
  //   }
  // }

  // // Activate or deactivate the processes by index
  // if (comptonIndex != -1) {
  //   pManager->SetProcessActivation(comptonIndex, false);  // 康普顿散射
  // }
  // if (photoIndex != -1) {
  //   pManager->SetProcessActivation(photoIndex, false);   // 光电效应
  // }
  // if (pairIndex != -1) {
  //   pManager->SetProcessActivation(pairIndex, false);    // 电子对效应
  // }
}
